...
export default function ShopContent() {
  ...
  const allPlaces = places.filter((p) => p.category === "shop");
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [allPlaces, region, sort]);

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">{ts("title")}</h1>
        <p className="text-gray-600 dark:text-gray-300">{ts("subtitle")}</p>
      </div>
