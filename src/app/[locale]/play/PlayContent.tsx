...
export default function PlayContent() {
  ...
  const allPlaces = places.filter((p) => p.category === "play");
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [allPlaces, region, age, indoor, sort]);

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">{tp("title")}</h1>
        <p className="text-gray-600 dark:text-gray-300">{tp("subtitle")}</p>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        <select value={indoor} onChange={(e) => setIndoor(e.target.value as typeof indoor)} className="px-3 py-2 rounded-lg border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm">
